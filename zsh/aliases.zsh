...
alias claude="AWS_ACCESS_KEY_ID=\$(op item get 'benji - claude on bedrock' --account=discourse --fields='access key id' --reveal) AWS_SECRET_ACCESS_KEY=\$(op item get 'benji - claude on bedrock' --account=discourse --fields='secret access key' --reveal) AWS_REGION=us-east-1 CLAUDE_CODE_USE_BEDROCK=1 claude"
alias agc="cd ~/dev/discourse/discourse/plugins/discourse-global-communities"
